document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Elementos de Marcos
    const marcosList = document.getElementById('dialog-marcos-list');
    const addMarcoContainer = document.getElementById('admin-add-marco-container');
    const newMarcoInput = document.getElementById('dialog-new-marco-input');
    const addMarcoBtn = document.getElementById('dialog-add-marco-btn');
    ...
    blocksData.forEach((data, index) => {
        ...
        for (let w = 1; w <= weeksPerBlock; w++) {
            ...
            const hasMarcos = semanaData.marcos && semanaData.marcos.length > 0;
            const milestoneIndicator = (isMilestone || hasMarcos) ? '<span class="milestone-indicator-star" title="Marco">🎯</span>' : '';
    ...
    document.querySelectorAll('.week-grid-card').forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (data.is_milestone || data.marcos.length > 0) {
            ...
            if (isAdmin) {
                ...
                addMarcoContainer.style.display = 'flex';
                ...
                addMarcoContainer.style.display = 'none';
            ...
            renderMarcosList(data.marcos, isAdmin, data.id, weekKey);

            weekModal.showModal();
        });
    });

    function renderMarcosList(marcos, isAdmin, semanaId, weekKey) {
        if (!marcosList) return;
        marcosList.innerHTML = '';
        
        if (!marcos || marcos.length === 0) {
            marcosList.innerHTML = '<span style="color:var(--text-muted); font-size: 0.9rem;">Nenhum marco registrado.</span>';
            return;
        }
        ...
        marcos.forEach(m => {
            ...
            desc.className = 'marco-item-desc';
            desc.innerText = m.descricao;
            
            item.appendChild(desc);

            if (isAdmin) {
                const actions = document.createElement('div');
                actions.className = 'marco-item-actions';
                
                const editBtn = document.createElement('button');
                editBtn.className = 'btn-icon';
                editBtn.innerHTML = '<span class="material-symbols-outlined icon-small">edit</span>';
                editBtn.title = 'Editar marco';
                editBtn.onclick = () => editMarco(m.id, m.descricao, semanaId, weekKey);
                
                const deleteBtn = document.createElement('button');
                deleteBtn.className = 'btn-icon delete';
                deleteBtn.innerHTML = '<span class="material-symbols-outlined icon-small">delete</span>';
                deleteBtn.title = 'Excluir marco';
                deleteBtn.onclick = () => deleteMarco(m.id, semanaId, weekKey);
            ...
            marcosList.appendChild(item);
        });
    }

    if (addMarcoBtn) {
        addMarcoBtn.addEventListener('click', async () => {
            const semanaId = saveWeekBtn.getAttribute('data-semana-id');
            const weekKey = saveWeekBtn.getAttribute('data-week-key');
            if (!semanaId) return;

            const desc = newMarcoInput.value.trim();
            if (!desc) {
                showToast('Digite a descrição do marco', 'error');
                return;
            }

            addMarcoBtn.disabled = true;
            try {
                const res = await fetch('/calendario/api/calendario/marco/add/', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRFToken': getCookie('csrftoken')
                    },
                    body: JSON.stringify({ semana_id: semanaId, descricao: desc })
                });
                const resData = await res.json();
                if (res.ok && resData.status === 'success') {
                    if (weeksCache[weekKey]) {
                        weeksCache[weekKey].marcos.push(resData.marco);
                    }
                    newMarcoInput.value = '';
                    renderMarcosList(weeksCache[weekKey].marcos, true, semanaId, weekKey);
                    updateWeekCardUI(weekKey);
                    showToast('Marco adicionado!', 'success');
                } else {
                    showToast(resData.error || 'Erro', 'error');
                }
            } catch (err) {
                showToast('Erro de conexão', 'error');
            } finally {
                addMarcoBtn.disabled = false;
            }
        });
    }

    async function editMarco(marcoId, currentDesc, semanaId, weekKey) {
        const newDesc = prompt("Editar Marco:", currentDesc);
        if (newDesc === null || newDesc.trim() === '' || newDesc.trim() === currentDesc) return;
        
        try {
            const res = await fetch('/calendario/api/calendario/marco/edit/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': getCookie('csrftoken')
                },
                body: JSON.stringify({ marco_id: marcoId, descricao: newDesc.trim() })
            });
            const resData = await res.json();
            if (res.ok && resData.status === 'success') {
                const mIndex = weeksCache[weekKey].marcos.findIndex(m => m.id === marcoId);
                if (mIndex > -1) {
                    weeksCache[weekKey].marcos[mIndex].descricao = resData.marco.descricao;
                }
                renderMarcosList(weeksCache[weekKey].marcos, true, semanaId, weekKey);
                showToast('Marco atualizado!', 'success');
            } else {
                showToast(resData.error || 'Erro', 'error');
            }
        } catch (err) {
    ...
    async function deleteMarco(marcoId, semanaId, weekKey) {
        if (!confirm("Tem certeza que deseja excluir este marco?")) return;
        try {
            const res = await fetch('/calendario/api/calendario/marco/delete/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': getCookie('csrftoken')
            ...
            const resData = await res.json();
            if (res.ok && resData.status === 'success') {
                weeksCache[weekKey].marcos = weeksCache[weekKey].marcos.filter(m => m.id !== marcoId);
                renderMarcosList(weeksCache[weekKey].marcos, true, semanaId, weekKey);
                updateWeekCardUI(weekKey);
    ...
    function updateWeekCardUI(weekKey) {
        const card = document.querySelector(`.week-grid-card[data-week-key="${weekKey}"]`);
        if (!card) return;
        const wData = weeksCache[weekKey];
        const hasMarcos = wData.marcos && wData.marcos.length > 0;
        
        if (wData.is_milestone || hasMarcos) {
            if (wData.is_milestone) {
            ...
            if (headerSmall) {
                headerSmall.innerHTML = `
                    <span class="week-number-small">S${wData.absoluteWeek}</span>
                    <span class="milestone-indicator-star" title="Marco">🎯</span>
                `;
            }
        } else {
            card.classList.remove('milestone-week');
