...
function updateLivrosTecnologia() {
    const today = new Date();
    // 17/06/2026
    const startDate = new Date(2026, 6, 3); // Month is 0-indexed in JS (5 is June)
    ...
    const counterElement = document.getElementById('counter-livros-tecnologia');
    console.log("updateLivrosTecnologia executou! dias: ", daysElapsed, " elemento: ", counterElement);
    if (counterElement) {
        counterElement.innerHTML = `${daysElapsed} <span class="unit">dias</span>`;
    }
}
